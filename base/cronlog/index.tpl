                        ...
                        <div class="tab-content" id="myTabContent" style="height: 800px;">
                            <div id="realtime" class="tab-pane fade active in">
                                ...
                                <div class="row-fluid">
                                    <div class="span12">
                                        <div class="widget">
                                            <div class="widget-header"><div class="title"><span class="fs1" aria-hidden="true" data-icon="&#xe07d;"></span> Latest Updates in <strong id="crontimer"></strong> secs</div></div>
                                            <div class="widget-body">
                                                <div id="scrollbar-three">
                                                    ...
                                                    <div class="viewport">
                                                        <div class="overview">
                                                            <ul class="imp-messages" id="cron-messages"></ul>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
...
function getUpdates(name,timerID, divID, timer)
{
    counter = $('#' + timerID).val();
    counter -= 1;
    ...
    if (counter <= 0)
    {
        ...
        $.ajax({
            type: "GET",
            url: "api/processlatestadmin?name=" + name + "&time=" + time,
            cache: false,
            success: function(xml) 
            {
                ...
                $(xml).find("line").each(function()
                {
                    ...
                    $('#' + divID).prepend('<li><div class="message-date"><h3 class="date text-info">' + dt + '</h3><p class="month">' + t + '</p></div><div class="message-wrapper"><h4 class="message-heading">' + $(this).text() + '</h4><p class="message"></p></div></li>');
                    
                    if ($('#' + divID + ' li').length > 20) 
                    {
                        $('#' + divID + ' li:last-child').remove();
                    }
                });
            }
        });
...
setInterval("getUpdates('cronjob', 'crontimer', 'cron-messages', 5)", 1000);
...
<?php $data = getD($dbCls, array(), array('links_parsed', 'links_unique')); ?>
        ['Date', '# Parsed', '# Unique'],
...
<?php $data = getD($dbCls, array(), array('links_failed', 'links_processed')); ?>
...
<?php $data = getD($dbCls, array(), array('emails_parsed', 'emails_unique')); ?>
        ['Date', '# Parsed', '# Unique'],
    <?php foreach ($data AS $date => $list) {?>
